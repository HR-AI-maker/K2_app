...
@section('title', 'Create Expedition')
...
<div class="bg-white rounded-lg shadow p-8 max-w-4xl">
    <form method="POST" action="{{ route('admin.expeditions.store') }}" class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                >
                    ...
                    <option value="Karakoram" {{ old('region') === 'Karakoram' ? 'selected' : '' }}>Karakoram</option>
                    <option value="Himalayas" {{ old('region') === 'Himalayas' ? 'selected' : '' }}>Himalayas</option>
                    <option value="Hindu Kush" {{ old('region') === 'Hindu Kush' ? 'selected' : '' }}>Hindu Kush</option>
                    <option value="Northern Areas" {{ old('region') === 'Northern Areas' ? 'selected' : '' }}>Northern Areas</option>
                </select>
                @error('region')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="flex gap-4 pt-4">
            ...
            <a
                href="{{ route('admin.expeditions.index') }}"
